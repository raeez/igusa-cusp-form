...
\cite{Mukai1984}) is the K3-side data; tensoring with the even
cohomology \(H^{\mathrm{even}}(E,\Z)=H^0(E,\Z)\oplus H^2(E,\Z)\)
(a hyperbolic plane under \(\langle 1_E,\omega_E\rangle=1\), the odd
K\"unneth term absent because \(S\) carries no odd cohomology) supplies
the even threefold lattice
...
normalization (computed/$\beta$, \cite{EZ:1}).  The topological Euler
...
Calabi--Yau dimension and the holomorphic part of the Mukai pairing.  The two integers do not
...
\Delta_5$ is conditional on four obstruction classes, each named and
discharged on $K3\times E$ in Appendix~\ref{app:obstruction-discharges}
(Theorems~\ref{thm:G-D0}, \ref{thm:G-O1}, \ref{thm:G-O1-plus},
\ref{thm:G-O2-orbit-transport}).  The chapter records them here as the
conditioning data feeding Chapter~\ref{chap:dirac-igusa-pro-object}.
...
(proved on $K3\times E$, Theorem~\ref{thm:G-D0}).
...
line $o_c$ across the $E$-quotient (proved on $K3\times E$,
...
$\epsilon_o = \nu_{\Delta_5}$ on the BKM Weyl group (proved on
$K3\times E$, Theorem~\ref{thm:G-O1-plus}).
...
(proved on $K3\times E$, Theorem~\ref{thm:G-O2-orbit-transport}).
\end{description}
