...
\((D0)\), \((O1)\), \((O1)^+\), \((O2)\) and discharged on the
\(K3\times E\) specialization in
...
\(\textcircled{2}\!\leftrightarrow\!\textcircled{3}\) unconditional at
the chiral-shadow level on \(K3\times E\); it relates the
...
each discharged on \(K3\times E\) in Appendix~\ref{app:obstruction-discharges}.
...
\(64^2 = 4096\), the squared theta-leading
\(([q^{1/2}r^{1/2}s^{1/2}]\Delta_5)^2\) of the Oberdieck--Pixton
...
attach to two distinct categorical dimensions: at the scalar-shadow level (the level-\(n\) trace of \(\mathfrak A_{K3\times E}\)) \(\Delta_5\) is the protected scalar shadow of
...
\(\mathcal P\to\mathbb D\), the mirror of \(K3\times E\) restricted to
its rank-one polarised subvariation, the discriminant locus is cut by
\[
  \mathrm{Disc}(\mathcal P\to\mathbb D) \;=\; \Delta_5^{-2}\;\big|_{\mathbb D},
\]
...
of the Beilinson chain of the wider research constellation. The
chiral source coalgebra \(C_X\) of Chapter~5.5 is chirally Koszul on
the log target \((K3\times E, D, \tau)\); Vol I's Theorem H supplies
the chain-level identification
\(Z^{\mathrm{der}}_{\mathrm{ch}}(C_X) = \mathrm{ChirHoch}^\bullet(C_X,C_X)\)
on the Koszul locus, and Vol II's chiral-Hochschild stage
stratification \textnormal{(i)+(ii)} supplies the CY-orientation trace
pairing of degree \(-\dim_{\mathbb C}(K3\times E) = -3\); these two
inputs together supply the level-\(\mathsf Z\) trace identity
\(\mathrm{Tr}^{\mathrm{bulk}}_n((-1)^F\cdot\mathrm{id})_{Z^{\mathrm{der}}_{\mathrm{ch}}(C_X)}
= -4096\,\Delta_5^{-2}\) of Appendix~\ref{app:obstruction-discharges},
Theorem~\ref{thm:G-vol2-discharge}. The level-\(\mathsf A\) promotion
to a gravity-line operator algebra acting on the boundary is Vol II's
Construction Problem~2, open. The Vol III census places
\(\mathfrak g_{\Delta_5}\) at the \((d=3,\,\text{rank }3)\) chamber
through the identity \(\kappa_{\mathrm{BKM}}(\Phi_N) = c_N(0)/2\),
where \(c_N(0)\) is the constant Fourier coefficient of the input
Jacobi form or Borcherds lift. Applied at the paramodular row
\((t,N;k) = (1,1;5)\) of the Gritsenko--Cl\'ery catalogue
\cite{GC, GN}, with input \(\Delta_5\), the identity reads
\(\kappa_{\mathrm{BKM}}(\Delta_5) = c_1(0)/2 = 5\). The cross-volume
companion at input \(\Phi_{12}\), the Fake-Monster Borcherds product
...
indexed by different input denominators, not in contradiction.
The mixed holomorphic-topological strings companion supplies the
...
at the Dirac brane vacuum; promotion of the level-\(n\) scalar shadow
\(Z^{K3\times E}_{\mathrm{BPS}} = \Delta_5^{-2}\) to a global compact
3d gravitational path integral on \(K3\times E\) requires both the
holomorphic de Rham obstruction class
\(\mathrm{ob}^{\mathrm{hol}}_{\mathrm{dR}}(K3\times E)\) to vanish and
the discharge of Vol II's Construction Problem~2; both conditions are
open.
