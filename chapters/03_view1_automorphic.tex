...
coefficient \(64\).  The \(\Sp_4(\Z)\)-equivariant data of the View~I
package are the multiplier system \(\nu_{\Delta_5}\), the Borcherds
...
\(\Sp_4\simeq\Spin(3,2)\) bridge of Chapter~\ref{ch:singular-theta-lift},
...
row \cite{CYQGVolIII}; the corrected entry of the
...
under the Borcherds-graded second-quantization; the full Sp(4)/Mp(4)/Howe
three-way crystallization sits in
...
compact Hall datum together with identification of that Pfaffian with
...
\(\La^{2,1}_{II}\) inside \(\La^{2,1}\): the type-II walls of the
...
\section{The \(\Sp_4\simeq\Spin(3,2)\) bridge to View~V}
