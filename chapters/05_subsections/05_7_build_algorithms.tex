...
\subsubsection*{Discharge status on \(K3\times E\)}

Algorithms A2--A4 at the test window are discharged by
Theorem~\ref{thm:G-D0}; A4 at the (O1) descent by
Theorem~\ref{thm:G-O1}; A4--A6 at (O1)$^+$ by
Theorem~\ref{thm:G-O1-plus}; A6 at (O2) by
Theorem~\ref{thm:G-O2-orbit-transport}; the A8 cofinal recognition
ledger at the first relation-closed window beyond $W_{\le 3}$ is the
open frontier of Chapter~\ref{ch:open-obstructions}.
