...
\cite{BBDJS2015} after classical truncation.  The holomorphic
symplectic two-form on the K3 fibre makes the unreduced
Donaldson--Thomas theory of $K3\times E$ vanish identically: it
generates trivial first-order deformations that obstruct the virtual
class.  The standard remedy, as in reduced K3 enumerative geometry
\cite{MaulikPandharipande}, is to quotient these out by a
semiregularity cosection.  The K3 fibre direction accordingly supplies
a semiregularity cosection $\operatorname{CS}_{\mathcal C}$, surjective
on reduced strata, whose kernel is the reduced obstruction theory.  On
...
\S\ref{subsec:hall-correspondences} — the cosections of the three
charge strata satisfy filtered Hall additivity
\[
q^*\operatorname{CS}_C
=p_1^*\operatorname{CS}_A+p_2^*\operatorname{CS}_B,
\]
the input to Kiem--Li cosection localization
...
null-trivializations.  Discharge of (O1) and (O1)$^+$ on
$K3\times E$ is the content of Theorems~\ref{thm:G-O1}
and~\ref{thm:G-O1-plus} of
Appendix~\ref{app:obstruction-discharges}; the K3-fibrewise framework
of \cite{JoyceSong,JoyceUpmeier} together with the Klein-four lemma
and $\mathbb P^3$-$\mathrm{Pin}^c$ lemma there reduces (O1) on the
projection-finite stratum, and the wrapped-stratum Weyl-equivariant
transport along $W^{(2)}(\La^{2,1}_{II})$ is established by the
$(3,3,3)$-abelianisation and Maass-character match.
