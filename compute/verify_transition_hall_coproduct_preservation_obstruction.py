#!/usr/bin/env python3
"""Verify the transition-Hall-coproduct preservation obstruction ledger.

This fail-closed verifier records the data missing from the proof that
finite-stage transition maps preserve the compact Hall coproduct.  A
positive result means the obstruction ledger is complete and the core
transition tables remain empty; it does not prove Hall-coproduct
preservation.
"""
...
SUCCESS_STATUS = "TRANSITION_HALL_COPRODUCT_PRESERVATION_OBSTRUCTION_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "transition_hall_coproduct_preservation_obstruction.v1"
EXPECTED_KIND = "transition_hall_coproduct_preservation_obstruction"
...
REQUIRED_OBLIGATIONS = frozenset(
    {
        ...
        "source_coproduct_matrix",
        "source_counit_map",
        "splitting_correspondence_square",
        "source_object_stack_square",
        "target_product_stack_square",
        "coefficient_system_transport",
        "ts_coproduct_transport",
        "proper_base_change",
        "projection_formula",
        "compact_support_pushforward",
        "coproduct_matrix_intertwining",
        "coassociativity_transition_compatibility",
        "counit_transition_compatibility",
        "hall_coproduct_mittag_leffler",
        "no_scalar_hall_coproduct_substitution",
    }
)
...
EMPTY_TABLE_SPECS: tuple[EmptyTableSpec, ...] = (
    EmptyTableSpec(
        ...
        (
            ...
            "from_stage",
            "to_stage",
            "source_object_stack_id",
            "target_product_stack_id",
            "source_splitting_stack_id",
            "target_splitting_stack_id",
            "source_square_id",
            "target_product_square_id",
            "transition_graph_id",
            "splitting_square_defect_rank",
            "proof_reference",
            "check_status",
            "notes",
        ),
    ...
    EmptyTableSpec(
        ...
        (
            ...
            "ts_coproduct_transport_id",
            "coefficient_system_defect_rank",
            "ts_inverse_defect_rank",
            "proof_reference",
            "check_status",
            "notes",
        ),
    ...
    EmptyTableSpec(
        ...
        (
            "matrix_transport_id",
            "transition_id",
            "source_coproduct_matrix_id",
            "target_coproduct_matrix_id",
            "input_transition_matrix_id",
            "left_output_transition_matrix_id",
            "right_output_transition_matrix_id",
    ...
    EmptyTableSpec(
        ...
        (
            ...
            "base_change_defect_rank",
            "projection_formula_defect_rank",
            "coproduct_intertwining_defect_rank",
            "coassociativity_transition_defect_rank",
            "counit_transition_defect_rank",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check transition-Hall-coproduct preservation obstruction ledger."
    )
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hall/transition_hall_coproduct_preservation"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "transition_hall_coproduct_preservation",
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected transition-Hall-coproduct tables")
...
def check_empty_tables(fixture: Path, issues: list[str]) -> None:
    for spec in EMPTY_TABLE_SPECS:
        ...
        if not path.is_file():
            issues.append(f"missing transition-Hall-coproduct table: {spec.path}")
            continue
        ...
        if rows:
            issues.append(
                ...
                "until Hall-coproduct preservation is proved"
            )
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    if missing:
        issues.append("missing Hall-coproduct obligations: " + ", ".join(sorted(missing)))
    if extra:
        issues.append("unexpected Hall-coproduct obligations: " + ", ".join(sorted(extra)))
    for obligation_id, row in by_id.items():
        if row.get("hall_coproduct_status") != "missing_open_obligation":
            issues.append(f"{obligation_id}: hall_coproduct_status is not missing_open_obligation")
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("TRANSITION_HALL_COPRODUCT_PRESERVATION_OBSTRUCTION_FAILED", file=sys.stderr)
        for issue in issues:
