#!/usr/bin/env python3
"""Verify the WWW associativity packet."""
...
SUCCESS_STATUS = "WWW_ASSOCIATIVITY_CONDITIONAL_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "www_associativity.v1"
...
EXPECTED_ASSOCIATIVITY = {
    "associativity_id": "assoc_WWW_R",
    "word": "WWW",
    "flag_stack_id": "F2_WWW_R",
    "left_comparison_id": "lambda_WWW",
    "right_comparison_id": "rho_WWW",
    "left_composite_id": "m_WW_zeta12_eta3_after_m_WW_eta1_eta2",
    "right_composite_id": "m_WW_eta1_zeta23_after_m_WW_eta2_eta3",
...
EXPECTED_FUNCTORIAL = {
    "www_flag_stack": ("flag_stack", "true"),
    "www_left_right_maps": ("flag_stack", "true"),
    "www_ww_target_admissibility": ("proper_target", "true"),
    "www_ww_base_change": ("base_change", "false"),
    "www_ww_projection_formula": ("projection_formula", "false"),
    "www_ww_ts": ("vanishing_cycles", "false"),
}
WW_HYPOTHESIS_ROWS = frozenset(
    {
        "www_ww_base_change",
        "www_ww_projection_formula",
        "www_ww_ts",
    }
)
...
EMPTY_TABLES = {
    "remaining_word_rows.csv": REMAINING_COLUMNS,
    "pentagon_rows.csv": (
        "pentagon_id",
        "word",
        "four_input_word",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check WWW associativity packet.")
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/www_associativity"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "www_associativity",
        ...
        "associativity_defect_rank": 0,
        "remaining_word_associativity_count": 0,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected WWW associativity tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected WWW imports")
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("WWW_ASSOCIATIVITY_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
