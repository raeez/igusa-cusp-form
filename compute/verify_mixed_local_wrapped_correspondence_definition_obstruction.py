#!/usr/bin/env python3
"""Verify the mixed local/wrapped correspondence definition packet."""
...
SUCCESS_STATUS = "MIXED_LOCAL_WRAPPED_CORRESPONDENCE_DEFINITION_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "mixed_local_wrapped_correspondence_definition.v1"
EXPECTED_KIND = "mixed_local_wrapped_correspondence_definition"
DEFINITION_COLUMNS = (
    "definition_id",
    "order",
    "local_input",
...
EXPECTED_DEFINITION_ROWS = {
    "mixed_LW_R_correspondence": {
        "order": "LW",
        "local_input": "alpha_I_to_Gamma_R_loc",
        "wrapped_input": "eta_in_Gamma_R_wr",
        "target_colour": "zeta_equals_sum_alpha_plus_eta_in_Gamma_R_wr",
        "source_carrier": "Ran_loc_R_pre_E_times_Ran_wr_R_pre_E",
        "target_carrier": "Ran_wr_R_pre_E",
        "extension_convention": "zero_to_W_eta_to_B_zeta_to_A_alpha_I_to_zero",
        "source_map": "p_LW_to_M_alpha_R_I_loc_cl_times_M_eta_R_wr_rig",
        "target_map": "q_LW_to_M_zeta_R_wr_rig",
    ...
    "mixed_WL_R_correspondence": {
        ...
        "local_input": "alpha_I_to_Gamma_R_loc",
        "wrapped_input": "eta_in_Gamma_R_wr",
        "target_colour": "zeta_equals_eta_plus_sum_alpha_in_Gamma_R_wr",
        "source_carrier": "Ran_wr_R_pre_E_times_Ran_loc_R_pre_E",
        "target_carrier": "Ran_wr_R_pre_E",
        "extension_convention": "zero_to_A_alpha_I_to_B_zeta_to_W_eta_to_zero",
        "source_map": "p_WL_to_M_eta_R_wr_rig_times_M_alpha_R_I_loc_cl",
        "target_map": "q_WL_to_M_zeta_R_wr_rig",
        "quotient_timing": "before_E_quotient",
        "excluded_substitutes": "no_quotient_first_no_scalar_factor",
        "check_status": "verified",
    },
...
EMPTY_TABLES = {
    "extension_stack_rows.csv": (
        ...
        "proper_or_admissible_defect_rank",
        "proof_reference",
        "check_status",
        "notes",
    ),
    ...
    "transition_rows.csv": (
        ...
        "from_R",
        "to_R",
        "mixed_correspondence_morphism_id",
...
REQUIRED_FIREWALL_ROWS = frozenset(
    {
        ...
        "local_local_as_mixed",
        "wrapped_wrapped_as_mixed",
        "two_sided_mixed_as_one_sided",
        "scalar_Fock_factor",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check mixed local/wrapped correspondence definition packet."
    )
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/mixed_local_wrapped_correspondence_definition"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "mixed_local_wrapped_correspondence_definition",
        "hybrid_kind": EXPECTED_KIND,
        "status": SUCCESS_STATUS,
        "mixed_correspondence_defined": True,
        "one_sided_only": True,
        ...
        "wrapped_wrapped_certification": False,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected mixed-correspondence tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected mixed-correspondence imports")
...
def check_definition(fixture: Path, issues: list[str]) -> None:
    ...
    if len(table.rows) != len(EXPECTED_DEFINITION_ROWS):
        issues.append("definition_rows.csv must contain exactly the LW and WL definition rows")
        return
    by_id = {row.get("definition_id", ""): row for row in table.rows}
    ...
    for definition_id, expected in EXPECTED_DEFINITION_ROWS.items():
        row = by_id.get(definition_id)
        if row is None:
            continue
        for key, value in expected.items():
            if row.get(key) != value:
                issues.append(
                    f"definition row {definition_id} {key}: expected {value!r}, got {row.get(key)!r}"
                )
        if not row.get("proof_reference"):
            issues.append(f"definition row {definition_id} lacks proof_reference")
...
def check_empty_tables(fixture: Path, issues: list[str]) -> None:
    for table_name, columns in EMPTY_TABLES.items():
        ...
        if table.rows:
            issues.append(f"{table_name} must remain empty until mixed-correspondence rows are supplied")
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    for index, row in enumerate(table.rows, start=2):
        if row.get("mixed_status") != "missing_open_obligation":
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("MIXED_LOCAL_WRAPPED_CORRESPONDENCE_DEFINITION_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
