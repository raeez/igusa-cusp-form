#!/usr/bin/env python3
"""Verify the row-359 finite Hall pairing-homogeneity packet."""
...
EXPECTED_SCHEMA = "finite_hall_pairing_homogeneity.v1"
EXPECTED_KIND = "finite_hall_pairing_homogeneity_obstruction"
SUCCESS_STATUS = "FINITE_HALL_PAIRING_HOMOGENEITY_OBSTRUCTION_VERIFIED"
PROOF_LABEL = "prop:finite-hall-pairing-homogeneity"
DEFAULT_FIXTURE = Path("certificates/hall/finite_hall_pairing_homogeneity")
...
REQUIRED_FIREWALL = {
    ...
    "denominator_product",
    "signed_dimension",
    "row358_definition_only",
    "row360_supersymmetry",
    "row361_invariance",
    "row362_coproduct",
    "empty_G_entries",
    "empty_homogeneity_rows",
}
...
TABLE_SPECS = (
    ...
    TableSpec(
        ...
        (
            ...
            "homogeneous_product_required",
            "degree_zero_trace_required",
            "normal_ordered_degree_required",
            "parity_block_required",
            "formal_pairing_rows_imported",
            "relative_theorem_recorded",
            "current_source_homogeneity_proved",
...
def check_manifest(fixture: Path) -> None:
    ...
    require_equal(manifest.get("row_number"), 359, "row_number")
    ...
    for key in (
        ...
        "source_homogeneity_defect_rows_supplied",
        "pairing_homogeneity_proved_for_current_source",
...
def check_local_tables(fixture: Path) -> None:
    ...
    if PROOF_LABEL not in row.get("proof_reference", ""):
        raise ValueError("criterion proof reference does not point to row-359 proposition")

    require_equal(len(tables["homogeneity_rows.csv"]), 0, "local homogeneity row count")
...
def check_pairing_pushforward_import() -> None:
    ...
    require_equal(manifest.get("schema_version"), "hall_pairing_pushforward_compatibility.v1", "pairing schema")
    ...
    require_false(manifest.get("hopf_adjointness"), "Hopf adjointness")
    require_false(manifest.get("frobenius_cyclic_identity"), "Frobenius cyclicity")
    require_false(manifest.get("quotient_nondegeneracy"), "quotient nondegeneracy")
    ...
    for row in pairing_rows:
        ...
        total_defect += int_cell(row, "degree_defect_rank")
        require_equal(row.get("check_status"), "verified", f"{row['pairing_id']} status")
    require_equal(total_defect, 0, "formal pairing total defect")
    ...
    if {"pairing_degree_compatible", "pushforward_pairing_graded", "hopf_adjointness_not_proved"} - relation_ids:
        raise ValueError("formal relations omit pairing homogeneity firewall rows")
...
def check_imports() -> None:
    ...
    require_false(pairing_definition.get("homogeneity_proved"), "row358 homogeneity")

    gram_degree = read_json(GRAM_DEGREE_FIXTURE / "manifest.json")
    require_equal(gram_degree.get("status"), GRAM_DEGREE_STATUS, "row356 packet status")
