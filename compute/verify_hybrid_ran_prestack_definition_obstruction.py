#!/usr/bin/env python3
"""Verify the hybrid Ran prestack definition packet."""
...
SUCCESS_STATUS = "HYBRID_RAN_PRESTACK_DEFINITION_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "hybrid_ran_prestack_definition.v1"
EXPECTED_KIND = "hybrid_ran_prestack_definition"
DEFINITION_COLUMNS = (
    "definition_id",
    "site",
    "functor_symbol",
    ...
    "symmetric_colimit",
    "stackification_rule",
    "degree_shadow_excluded",
    "proof_reference",
    "check_status",
    "notes",
)
EMPTY_TABLES = {
    "component_prestack_rows.csv": (
        "component_id",
        "R_id",
        "component_kind",
        "index_set_id",
    ...
    "transition_rows.csv": (
        ...
        "from_R",
        "to_R",
        "prestack_morphism_id",
        "local_component_compatibility_defect_rank",
        "wrapped_component_compatibility_defect_rank",
...
REQUIRED_OBLIGATIONS = frozenset(
    {
        "local_component_prestack",
        "wrapped_component_prestack",
        "symmetric_relabelling",
        "descent_rows",
        "stackification_rows",
        "transition_rows",
        "support_realization_import",
    }
)
REQUIRED_FIREWALL_ROWS = frozenset(
    {
        "degree_shadow_disjoint_union",
        "ordinary_Ran_only",
        "scalar_degree_attached_to_Ran",
        "wrapped_points_as_set",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check hybrid Ran prestack definition packet."
    )
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/hybrid_ran_prestack_definition"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "hybrid_ran_prestack_definition",
        "hybrid_kind": EXPECTED_KIND,
        "status": SUCCESS_STATUS,
        "prestack_defined": True,
        "stackification_certification": False,
        "descent_certification": False,
        "local_stratum_certification": False,
        "wrapped_stratum_certification": False,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected prestack tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected prestack imports")
...
def check_definition(fixture: Path, issues: list[str]) -> None:
    ...
    row = table.rows[0]
    expected = {
        "definition_id": "hybrid_ran_R_prestack",
        "site": "Aff_C_etale",
        "functor_symbol": "Ran_hyb_R_pre_E",
        "test_object": "T",
        "stackification_rule": "stackification only after descent rows supplied",
        "degree_shadow_excluded": "true",
    ...
    for needle in (
        "finite maps T_to_E",
        "T_families in rigidified wrapped prequotients",
        "colimit over finite index sets",
    ):
...
def check_empty_tables(fixture: Path, issues: list[str]) -> None:
    for table_name, columns in EMPTY_TABLES.items():
        ...
        if table.rows:
            issues.append(f"{table_name} must remain empty until component/descent rows are supplied")
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    for index, row in enumerate(table.rows, start=2):
        if row.get("prestack_status") != "missing_open_obligation":
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("HYBRID_RAN_PRESTACK_DEFINITION_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
