#!/usr/bin/env python3
"""Verify the row-345 finite Hall product compactification-independence packet."""
...
EXPECTED_SCHEMA = "finite_hall_product_compactification_independence_obstruction.v1"
EXPECTED_KIND = "finite_hall_product_compactification_independence_obstruction"
SUCCESS_STATUS = "FINITE_HALL_PRODUCT_COMPACTIFICATION_INDEPENDENCE_OBSTRUCTION_VERIFIED"
PROOF_LABEL = "prop:finite-hall-product-qbang-compactification-independence"
DEFAULT_FIXTURE = Path("certificates/hall/finite_hall_product_compactification_independence")
...
REQUIRED_OBLIGATIONS = {
    "retained_q_map",
    "compactification_pair",
    "common_proper_refinement",
    "extension_by_zero_comparison",
    "choice_independence_row",
    "input_coefficient_constructible",
    "row344_existence_not_independence",
    "compact_support_definition_not_independence",
    "product_matrix_rows",
}
...
TABLE_SPECS = (
    ...
    TableSpec(
        ...
        (
            "criterion_id",
            "retained_q_map_required",
            "two_compactifications_required",
            "common_proper_refinement_required",
            "extension_by_zero_comparison_required",
            "chosen_six_functor_formalism_required",
            "input_coefficient_constructible_required",
            "criterion_recorded",
            "actual_independence_proved",
            "q_bang_existence_claimed",
            "base_change_claimed",
            "projection_formula_claimed",
    ...
    TableSpec(
        "independence_rows.csv",
        (
            "independence_row_id",
            "R_id",
            "q_map_id",
            "input_coefficient_id",
            "compactification_left_id",
            "compactification_right_id",
            "common_refinement_id",
            "left_comparison_isomorphism_id",
            "right_comparison_isomorphism_id",
            "independence_isomorphism_id",
...
def check_manifest(fixture: Path) -> None:
    ...
    for key in (
        ...
        "common_refinement_criterion_recorded",
        "compact_support_comparison_formula_recorded",
    ):
        require_true(manifest.get(key), key)

    for key in (
        "retained_q_map_rows_supplied",
        "compactification_pair_rows_supplied",
        "common_refinement_rows_supplied",
        "comparison_isomorphism_rows_supplied",
        "choice_independence_rows_supplied",
        "actual_q_bang_independence_proved",
...
def check_local_tables(fixture: Path) -> None:
    ...
    if PROOF_LABEL not in row.get("proof_reference", ""):
        raise ValueError("criterion proof reference does not point to row-345 proposition")

    require_equal(len(tables["independence_rows.csv"]), 0, "independence row count")
...
def check_imports() -> None:
    ...
    require_false(product.get("compactification_independence_proved"), "row342 independence")
    ...
    require_false(
        pushforward.get("compactification_independence_proved"),
        "row344 independence",
    )
    require_false(pushforward.get("actual_q_bang_leg_proved"), "row344 actual q bang")
