#!/usr/bin/env python3
"""Verify the WWL associativity packet."""
...
SUCCESS_STATUS = "WWL_ASSOCIATIVITY_CONDITIONAL_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "wwl_associativity.v1"
...
EXPECTED_ASSOCIATIVITY = {
    "associativity_id": "assoc_WWL_R",
    "word": "WWL",
    "flag_stack_id": "F2_WWL_R",
    "left_comparison_id": "lambda_WWL",
    "right_comparison_id": "rho_WWL",
    "left_composite_id": "m_WL_zeta12_alpha_after_m_WW_eta1_eta2",
    "right_composite_id": "m_WW_eta1_zeta23_after_m_WL_eta2_alpha",
...
EXPECTED_FUNCTORIAL = {
    "wwl_flag_stack": ("flag_stack", "true"),
    "wwl_left_right_maps": ("flag_stack", "true"),
    "wwl_mixed_base_change": ("base_change", "true"),
    "wwl_mixed_projection_formula": ("projection_formula", "true"),
    "wwl_mixed_ts": ("vanishing_cycles", "true"),
    "wwl_ww_target_admissibility": ("proper_target", "true"),
    "wwl_ww_base_change": ("base_change", "false"),
    "wwl_ww_projection_formula": ("projection_formula", "false"),
    "wwl_ww_ts": ("vanishing_cycles", "false"),
}
WW_HYPOTHESIS_ROWS = frozenset(
    {
        "wwl_ww_base_change",
        "wwl_ww_projection_formula",
        "wwl_ww_ts",
    }
)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check WWL associativity packet.")
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/wwl_associativity"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "associativity_defect_rank": 0,
        "remaining_word_associativity_count": 1,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected WWL associativity tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected WWL imports")
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("WWL_ASSOCIATIVITY_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
