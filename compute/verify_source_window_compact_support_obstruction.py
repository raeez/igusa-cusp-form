#!/usr/bin/env python3
"""Verify the source-window compact-support obstruction ledger.

This verifier records the exact missing data for source windows to
exhaust compact support.  It is fail-closed.  A positive result means
the obstruction ledger is complete and the core source tables remain
empty; it does not prove compact-source support
exhaustion.
"""
...
SUCCESS_STATUS = "SOURCE_WINDOW_COMPACT_SUPPORT_OBSTRUCTION_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "source_window_compact_support_obstruction.v1"
EXPECTED_KIND = "source_window_compact_support_obstruction"
EXPECTED_EMPTY_TABLES = (
    "compact_support_indices.csv",
    "source_windows.csv",
    "support_exhaustion.csv",
    "transitions.csv",
)
OBLIGATION_COLUMNS = (
    "obligation_id",
...
REQUIRED_OBLIGATIONS = frozenset(
    {
        "compact_support_index_set",
        "source_height_function",
        "finite_source_windows",
        "compact_provenance_rows",
        "cofinality_witness",
        "source_window_transitions",
        "target_source_compatibility",
        "no_target_label_substitution",
    }
)
...
EMPTY_TABLE_SPECS: tuple[EmptyTableSpec, ...] = (
    ...
    EmptyTableSpec(
        ...
        (
            ...
            "source_window_from",
            "source_window_to",
            "closed_embedding_or_proper_map",
            "strict_inclusion",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check source-window compact-support obstruction ledger.")
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/sources/source_window_compact_support"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "source_window_compact_support",
        "source_kind": EXPECTED_KIND,
        "status": SUCCESS_STATUS,
        "compact_support_exhaustion": False,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected source compact-support tables")
...
def check_empty_tables(fixture: Path, issues: list[str]) -> None:
    for spec in EMPTY_TABLE_SPECS:
        ...
        if not path.is_file():
            issues.append(f"missing source compact-support table: {spec.path}")
            continue
        ...
        if rows:
            issues.append(f"{spec.path}: obstruction packet must keep core table empty until source compact-support exhaustion is proved")
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    if missing:
        issues.append("missing support obligations: " + ", ".join(sorted(missing)))
    if extra:
        issues.append("unexpected support obligations: " + ", ".join(sorted(extra)))
    for obligation_id, row in by_id.items():
        if row.get("support_status") != "missing_open_obligation":
            issues.append(f"{obligation_id}: support_status is not missing_open_obligation")
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("SOURCE_WINDOW_COMPACT_SUPPORT_OBSTRUCTION_FAILED", file=sys.stderr)
        for issue in issues:
