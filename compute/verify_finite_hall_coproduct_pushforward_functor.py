#!/usr/bin/env python3
"""Verify the row-348 finite Hall coproduct pushforward-functor packet."""
...
EXPECTED_SCHEMA = "finite_hall_coproduct_pushforward_functor_obstruction.v1"
EXPECTED_KIND = "finite_hall_coproduct_pushforward_functor_obstruction"
SUCCESS_STATUS = "FINITE_HALL_COPRODUCT_PUSHFORWARD_FUNCTOR_OBSTRUCTION_VERIFIED"
PROOF_LABEL = "prop:finite-hall-coproduct-pushforward-existence"
DEFAULT_FIXTURE = Path("certificates/hall/finite_hall_coproduct_pushforward_functor")
...
REQUIRED_OBLIGATIONS = {
    "retained_splitting_stack",
    "p_map_row",
    "p_map_proper_or_compactified",
    "compact_support_model_population",
    "input_coefficient_constructible",
    "inverse_thom_sebastiani_row",
    "pushforward_row",
    "compactification_independence_separate",
    "row347_notation_not_pbang",
...
TABLE_SPECS = (
    ...
    TableSpec(
        ...
        (
            "criterion_id",
            "retained_splitting_correspondence_required",
            "p_map_required",
            "p_map_proper_or_compactified_required",
...
def check_manifest(fixture: Path) -> None:
    ...
    for key in (
        ...
        "p_map_proper_or_compactified_rows_supplied",
...
def check_local_tables(fixture: Path) -> None:
    ...
    if PROOF_LABEL not in row.get("proof_reference", ""):
        raise ValueError("criterion proof reference does not point to row-348 proposition")
...
def check_imports() -> None:
    ...
    require_false(coproduct.get("pushforward_exists_proved"), "row347 pushforward proof")
    ...
    require_equal(
        {row.get("operation_id") for row in operation_rows},
        {"target_pushforward_q", "source_pushforward_p"},
        "compact support operation ids",
    )
    for table_name in (
    ...
    for table_name in (
        ...
        require_table_empty(TRANSITION_COPRODUCT_FIXTURE / table_name)
