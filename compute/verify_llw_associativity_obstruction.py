#!/usr/bin/env python3
"""Verify the LLW associativity packet."""
...
SUCCESS_STATUS = "LLW_ASSOCIATIVITY_CONDITIONAL_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "llw_associativity.v1"
...
EXPECTED_ASSOCIATIVITY = {
    "associativity_id": "assoc_LLW_R",
    "word": "LLW",
    "flag_stack_id": "F2_LLW_R",
    "left_comparison_id": "lambda_LLW",
    "right_comparison_id": "rho_LLW",
    "left_composite_id": "m_LW_gamma12_eta_after_m_LL_alpha_beta",
    "right_composite_id": "m_LW_alpha_zeta23_after_m_LW_beta_eta",
...
EXPECTED_FUNCTORIAL = {
    "llw_flag_stack": ("flag_stack", "true"),
    "llw_left_right_maps": ("flag_stack", "true"),
    "llw_local_base_change": ("base_change", "false"),
    "llw_mixed_base_change": ("base_change", "true"),
    "llw_projection_formula": ("projection_formula", "false"),
    "llw_mixed_ts": ("vanishing_cycles", "true"),
}
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check LLW associativity packet.")
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/llw_associativity"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "llw_associativity",
        ...
        "associativity_defect_rank": 0,
        "remaining_word_associativity_count": 6,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected LLW associativity tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected LLW imports")
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    for index, row in enumerate(table.rows, start=2):
        if row.get("llw_status") != "missing_open_obligation":
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("LLW_ASSOCIATIVITY_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
