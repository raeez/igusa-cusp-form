#!/usr/bin/env python3
"""Verify the row-344 finite Hall product pushforward-functor packet."""
...
EXPECTED_SCHEMA = "finite_hall_product_pushforward_functor_obstruction.v1"
EXPECTED_KIND = "finite_hall_product_pushforward_functor_obstruction"
SUCCESS_STATUS = "FINITE_HALL_PRODUCT_PUSHFORWARD_FUNCTOR_OBSTRUCTION_VERIFIED"
PROOF_LABEL = "prop:finite-hall-product-pushforward-existence"
DEFAULT_FIXTURE = Path("certificates/hall/finite_hall_product_pushforward_functor")

PRODUCT_OPERATOR_FIXTURE = Path("certificates/hall/finite_hall_product_operator")
PULLBACK_FIXTURE = Path("certificates/hall/finite_hall_product_pullback_functor")
...
PULLBACK_STATUS = "FINITE_HALL_PRODUCT_PULLBACK_FUNCTOR_OBSTRUCTION_VERIFIED"
...
REQUIRED_OBLIGATIONS = {
    ...
    "q_map_row",
    "q_map_proper_or_compactified",
    "compact_support_model_population",
    "input_coefficient_constructible",
    "reduced_thom_sebastiani_row",
    "pushforward_row",
    "compactification_independence_separate",
    "row342_notation_not_qbang",
    "row343_pullback_not_qbang",
    "product_matrix_rows",
}
...
TABLE_SPECS = (
    ...
    TableSpec(
        ...
        (
            ...
            "q_map_required",
            "q_map_proper_or_compactified_required",
            "chosen_six_functor_formalism_required",
            "input_coefficient_constructible_required",
            "extension_by_zero_or_proper_pushforward_required",
            "criterion_recorded",
            "actual_q_bang_leg_proved",
    ...
    TableSpec(
        ...
        (
            ...
            "pushforward_coefficient_id",
            "q_proper_or_compactified",
            "pushforward_exists",
...
def check_local_tables(fixture: Path) -> None:
    ...
    if PROOF_LABEL not in row.get("proof_reference", ""):
        raise ValueError("criterion proof reference does not point to row-344 proposition")

    require_equal(len(tables["pushforward_rows.csv"]), 0, "pushforward row count")
...
def check_imports() -> None:
    ...
    require_false(product.get("pushforward_exists_proved"), "row342 pushforward proof")
    ...
    pullback = read_json(PULLBACK_FIXTURE / "manifest.json")
    require_equal(pullback.get("status"), PULLBACK_STATUS, "pullback status")
    require_false(pullback.get("actual_p_star_leg_proved"), "row343 actual pullback")
    require_false(pullback.get("finite_hall_product_populated"), "row343 populated product")
    require_table_empty(PULLBACK_FIXTURE / "pullback_rows.csv")
