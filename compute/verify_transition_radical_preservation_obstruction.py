#!/usr/bin/env python3
"""Verify the transition-radical preservation obstruction ledger.

This fail-closed verifier records the data missing from the proof that
finite-stage transition maps preserve the Hopf-pairing radical.  A
positive result means the obstruction ledger is complete and the core
transition tables remain empty; it does not prove radical preservation.
"""
...
SUCCESS_STATUS = "TRANSITION_RADICAL_PRESERVATION_OBSTRUCTION_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "transition_radical_preservation_obstruction.v1"
EXPECTED_KIND = "transition_radical_preservation_obstruction"
...
REQUIRED_OBLIGATIONS = frozenset(
    {
        ...
        "target_hopf_pairing_identities",
        "source_radical_kernel",
        "target_radical_kernel",
        "opposite_primitive_lift",
        "pairing_transition_identity",
        "radical_image_identity",
        "radical_restricted_transition_matrix",
        "source_quotient_splitting",
        "target_quotient_splitting",
        "quotient_descended_transition_matrix",
        "radical_transition_composition",
        "no_scalar_radical_substitution",
    }
)
...
EMPTY_TABLE_SPECS: tuple[EmptyTableSpec, ...] = (
    EmptyTableSpec(
        ...
        (
            ...
            "target_pairing_matrix_id",
            "positive_primitive_transition_id",
            "negative_primitive_lift_id",
            "pairing_intertwining_defect_rank",
            "lift_surjectivity_defect_rank",
            "proof_reference",
            "check_status",
            "notes",
        ),
    ...
    EmptyTableSpec(
        ...
        (
            ...
            "from_stage",
            "to_stage",
            "ambient_primitive_transition_id",
            "source_radical_basis_id",
            "target_radical_basis_id",
            "restricted_radical_transition_id",
            "source_inclusion_id",
            "target_inclusion_id",
            "proof_reference",
            "check_status",
    ...
    EmptyTableSpec(
        ...
        (
            ...
            "transition_id",
            "source_quotient_splitting_id",
            "target_quotient_splitting_id",
            "descended_transition_matrix_id",
            "quotient_well_defined_defect_rank",
            "splitting_compatibility_defect_rank",
            "proof_reference",
            "check_status",
            "notes",
        ),
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check transition-radical preservation obstruction ledger."
    )
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hall/transition_radical_preservation"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "transition_radical_preservation",
        "hall_kind": EXPECTED_KIND,
        "status": SUCCESS_STATUS,
        "radical_transition_certification": False,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected transition-radical tables")
...
def check_empty_tables(fixture: Path, issues: list[str]) -> None:
    for spec in EMPTY_TABLE_SPECS:
        ...
        if not path.is_file():
            issues.append(f"missing transition-radical table: {spec.path}")
            continue
        ...
        if rows:
            issues.append(
                ...
                "until radical preservation is proved"
            )
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    if missing:
        issues.append("missing radical-transition obligations: " + ", ".join(sorted(missing)))
    if extra:
        issues.append("unexpected radical-transition obligations: " + ", ".join(sorted(extra)))
    for obligation_id, row in by_id.items():
        if row.get("radical_transition_status") != "missing_open_obligation":
            issues.append(f"{obligation_id}: radical_transition_status is not missing_open_obligation")
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("TRANSITION_RADICAL_PRESERVATION_OBSTRUCTION_FAILED", file=sys.stderr)
        for issue in issues:
