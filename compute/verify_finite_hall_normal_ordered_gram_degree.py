#!/usr/bin/env python3
"""Verify the row-356 finite Hall normal-ordered Gram-degree packet."""
...
EXPECTED_SCHEMA = "finite_hall_normal_ordered_gram_degree.v1"
EXPECTED_KIND = "finite_hall_normal_ordered_gram_degree_obstruction"
SUCCESS_STATUS = "FINITE_HALL_NORMAL_ORDERED_GRAM_DEGREE_OBSTRUCTION_VERIFIED"
PROOF_LABEL = "prop:finite-hall-normal-ordered-gram-degree"
DEFAULT_FIXTURE = Path("certificates/hall/finite_hall_normal_ordered_gram_degree")

JACOBI_FIXTURE = Path("certificates/hall/finite_hall_graded_jacobi")
PUSHFORWARD_FIXTURE = Path("certificates/charge/hall_pairing_pushforward_compatibility")
...
EXPECTED_COVERAGE = {
    ...
    "formal_bracket_degree_defect_total": 0,
    "formal_pairing_rows": 3,
    "compact_source_degree_rows": 0,
    "source_normal_ordered_lift_rows": 0,
    "source_gram_map_rows": 0,
    "source_product_matrix_rows": 0,
    "source_primitive_bracket_rows": 0,
    "source_degree_defect_rows": 0,
}

EXPECTED_FORMAL_IMPORTS = {
    "formal_bracket_rows": ("bracket_rows.csv", 3, 3, 0),
    "formal_pairing_rows": ("pairing_rows.csv", 3, 3, 0),
    "formal_relations": ("formal_relations.csv", 6, 6, 0),
}

REQUIRED_OBLIGATIONS = {
    "source_degree_rows",
    "normal_ordered_lift_rows",
    "additive_normal_ordering_rows",
    "homogeneous_hall_product_rows",
    "primitive_bracket_matrix",
    "bracket_degree_defect_rows",
    "fibre_sum_basis_rows",
    "row357_parity_rows",
...
REQUIRED_FIREWALL = {
    "target_lie_algebra",
    "target_degree_closure",
    "scalar_trace",
    "formal_pushforward_rows_only",
    "row355_jacobi_only",
    "raw_quadratic_pi",
    "pairing_degree_only",
    "bar_pushforward_only",
    "degree_without_bracket",
    "parity_shortcut",
    "normal_ordering_without_source_lifts",
}
...
TABLE_SPECS = (
    ...
    TableSpec(
        ...
        (
            "criterion_id",
            "additive_normal_ordering_required",
            "homogeneous_bracket_required",
            "bracket_degree_additivity_required",
            "fibre_sum_definition_required",
            "formal_pushforward_packet_imported",
            "relative_theorem_recorded",
            "current_source_degree_proved",
    ...
    TableSpec(
        ...
        (
            "degree_row_id",
            "R_id",
            "left_source_degree_id",
            "right_source_degree_id",
            "target_source_degree_id",
            "left_normal_ordered_degree",
            "right_normal_ordered_degree",
            "target_normal_ordered_degree",
            "degree_defect_rank",
            "degree_preservation_verified",
            "proof_reference",
            "check_status",
            "notes",
        ),
...
def check_manifest(fixture: Path) -> None:
    ...
    require_equal(manifest.get("row_number"), 356, "row_number")

    for key in (
        "relative_theorem_recorded",
        "graded_jacobi_packet_imported",
        "pushforward_degree_packet_imported",
        "charge_window_fixture_imported",
        "compact_hall_source_ledger_imported",
        "normal_ordered_degree_formula_recorded",
    ...
    for key in (
        ...
        "source_normal_ordered_lift_rows_supplied",
        "source_product_matrix_rows_supplied",
        "source_primitive_bracket_rows_supplied",
        "source_bracket_degree_defect_rows_supplied",
        "normal_ordered_gram_degree_proved_for_current_source",
        "primitive_lie_algebra_populated",
        "parity_proved",
...
def check_local_tables(fixture: Path) -> None:
    ...
    if PROOF_LABEL not in row.get("proof_reference", ""):
        raise ValueError("criterion proof reference does not point to row-356 proposition")

    require_equal(len(tables["degree_rows.csv"]), 0, "degree row count")
...
def check_pushforward_import() -> None:
    manifest = read_json(PUSHFORWARD_FIXTURE / "manifest.json")
    require_equal(
        manifest.get("schema_version"),
        "hall_pairing_pushforward_compatibility.v1",
        "pushforward schema",
    )
    require_true(manifest.get("certified"), "pushforward certified")
    ...
    for key in (
        ...
        require_false(manifest.get(key), f"pushforward {key}")

    bracket_rows = read_table_path(PUSHFORWARD_FIXTURE / "bracket_rows.csv")
    require_equal(len(bracket_rows), 3, "pushforward bracket row count")
    for row in bracket_rows:
        for axis in ("n", "l", "m"):
            require_equal(
                int_cell(row, f"target_degree_{axis}"),
                int_cell(row, f"sum_degree_{axis}"),
                f"{row['bracket_id']} target=sum {axis}",
            )
        require_equal(int_cell(row, "degree_defect_rank"), 0, f"{row['bracket_id']} defect")
        require_equal(row.get("check_status"), "verified", f"{row['bracket_id']} status")

    pairing_rows = read_table_path(PUSHFORWARD_FIXTURE / "pairing_rows.csv")
    require_equal(len(pairing_rows), 3, "pushforward pairing row count")
    ...
    formal_relations = read_table_path(PUSHFORWARD_FIXTURE / "formal_relations.csv")
    require_equal(len(formal_relations), 6, "pushforward formal relation count")
    relation_ids = {row["relation_id"] for row in formal_relations}
    if {"bracket_degree_compatible", "pushforward_bracket_graded"} - relation_ids:
        raise ValueError("pushforward relations omit bracket degree rows")
...
def check_imports() -> None:
    ...
    require_false(
        jacobi.get("normal_ordered_bracket_rows_supplied"),
        "row355 normal ordered bracket rows",
    )
    require_false(
        jacobi.get("graded_jacobi_proved_for_current_source"),
        "row355 current-source Jacobi",
    )

    check_pushforward_import()

    charge_window = read_json(CHARGE_WINDOW_FIXTURE / "manifest.json")
    require_equal(charge_window.get("charge_kind"), "mock_empty_blocked", "charge window kind")
    require_false(charge_window.get("certified"), "charge window certified")
    require_true(charge_window.get("empty_blocked"), "charge window empty blocked")
    require_table_empty(CHARGE_WINDOW_FIXTURE / "normal_ordered_lifts.csv")
    require_table_empty(CHARGE_WINDOW_FIXTURE / "gram_map.csv")
