#!/usr/bin/env python3
"""Verify the LWW associativity packet."""
...
SUCCESS_STATUS = "LWW_ASSOCIATIVITY_CONDITIONAL_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "lww_associativity.v1"
...
EXPECTED_ASSOCIATIVITY = {
    "associativity_id": "assoc_LWW_R",
    "word": "LWW",
    "flag_stack_id": "F2_LWW_R",
    "left_comparison_id": "lambda_LWW",
    "right_comparison_id": "rho_LWW",
    "left_composite_id": "m_WW_zeta12_eta2_after_m_LW_alpha_eta1",
    "right_composite_id": "m_LW_alpha_zeta23_after_m_WW_eta1_eta2",
...
EXPECTED_FUNCTORIAL = {
    "lww_flag_stack": ("flag_stack", "true"),
    "lww_left_right_maps": ("flag_stack", "true"),
    "lww_mixed_base_change": ("base_change", "true"),
    "lww_mixed_projection_formula": ("projection_formula", "true"),
    "lww_mixed_ts": ("vanishing_cycles", "true"),
    "lww_ww_target_admissibility": ("proper_target", "true"),
    "lww_ww_base_change": ("base_change", "false"),
    "lww_ww_projection_formula": ("projection_formula", "false"),
    "lww_ww_ts": ("vanishing_cycles", "false"),
}
WW_HYPOTHESIS_ROWS = frozenset(
    {
        "lww_ww_base_change",
        "lww_ww_projection_formula",
        "lww_ww_ts",
    }
)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check LWW associativity packet.")
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/lww_associativity"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "associativity_defect_rank": 0,
        "remaining_word_associativity_count": 3,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected LWW associativity tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected LWW imports")
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("LWW_ASSOCIATIVITY_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
