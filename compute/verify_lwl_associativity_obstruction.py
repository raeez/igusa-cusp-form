#!/usr/bin/env python3
"""Verify the LWL associativity packet."""
...
SUCCESS_STATUS = "LWL_ASSOCIATIVITY_CONDITIONAL_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "lwl_associativity.v1"
...
EXPECTED_ASSOCIATIVITY = {
    "associativity_id": "assoc_LWL_R",
    "word": "LWL",
    "flag_stack_id": "F2_LWL_R",
    "left_comparison_id": "lambda_LWL",
    "right_comparison_id": "rho_LWL",
    "left_composite_id": "m_WL_zeta12_beta_after_m_LW_alpha_eta",
    "right_composite_id": "m_LW_alpha_zeta23_after_m_WL_eta_beta",
...
EXPECTED_FUNCTORIAL = {
    "lwl_flag_stack": ("flag_stack", "true"),
    "lwl_left_right_maps": ("flag_stack", "true"),
    "lwl_mixed_base_change": ("base_change", "true"),
    "lwl_mixed_projection_formula": ("projection_formula", "true"),
    "lwl_mixed_ts": ("vanishing_cycles", "true"),
}
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check LWL associativity packet.")
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/lwl_associativity"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "lwl_associativity",
        ...
        "associativity_defect_rank": 0,
        "remaining_word_associativity_count": 5,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected LWL associativity tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected LWL imports")
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    for index, row in enumerate(table.rows, start=2):
        if row.get("lwl_status") != "missing_open_obligation":
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("LWL_ASSOCIATIVITY_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
