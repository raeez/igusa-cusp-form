#!/usr/bin/env python3
"""Verify the local/local correspondence definition packet."""
...
SUCCESS_STATUS = "LOCAL_LOCAL_CORRESPONDENCE_DEFINITION_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "local_local_correspondence_definition.v1"
EXPECTED_KIND = "local_local_correspondence_definition"
DEFINITION_COLUMNS = (
    "definition_id",
    "order",
    "local_left_input",
...
EMPTY_TABLES = {
    ...
    "transition_rows.csv": (
        ...
        "from_R",
        "to_R",
        "local_local_correspondence_morphism_id",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check local/local correspondence definition packet."
    )
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/local_local_correspondence_definition"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "local_local_correspondence_definition",
        "hybrid_kind": EXPECTED_KIND,
        "status": SUCCESS_STATUS,
        "local_local_correspondence_defined": True,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected local/local tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected local/local imports")
...
def check_definition(fixture: Path, issues: list[str]) -> None:
    ...
    if len(table.rows) != 1:
        issues.append("definition_rows.csv must contain exactly one local/local row")
        return
    row = table.rows[0]
    expected = {
        "definition_id": "local_local_R_correspondence",
        "order": "LL",
        "local_left_input": "alpha_I_to_Gamma_R_loc",
        "local_right_input": "beta_J_to_Gamma_R_loc",
        "target_colour": "gamma_equals_sum_alpha_plus_sum_beta_in_Gamma_R_loc",
        "source_carrier": "Ran_loc_R_pre_E_times_Ran_loc_R_pre_E",
        "target_carrier": "Ran_loc_R_pre_E",
        "extension_convention": "zero_to_A_beta_J_to_B_gamma_K_to_A_alpha_I_to_zero",
        "source_map": "p_LL_to_M_alpha_R_I_loc_cl_times_M_beta_R_J_loc_cl",
        "target_map": "q_LL_to_M_gamma_R_K_loc_cl",
        "collision_rule": "K_is_I_disjoint_union_J_mod_Ran_collisions",
        "quotient_timing": "no_E_quotient",
        "excluded_substitutes": "no_wrapped_inputs_no_scalar_factor",
...
def check_empty_tables(fixture: Path, issues: list[str]) -> None:
    for table_name, columns in EMPTY_TABLES.items():
        ...
        if table.rows:
            issues.append(f"{table_name} must remain empty until local/local rows are supplied")
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    for index, row in enumerate(table.rows, start=2):
        if row.get("local_local_status") != "missing_open_obligation":
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("LOCAL_LOCAL_CORRESPONDENCE_DEFINITION_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
