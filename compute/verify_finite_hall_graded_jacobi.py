#!/usr/bin/env python3
"""Verify the row-355 finite Hall graded-Jacobi packet."""
...
EXPECTED_SCHEMA = "finite_hall_graded_jacobi.v1"
EXPECTED_KIND = "finite_hall_graded_jacobi_obstruction"
SUCCESS_STATUS = "FINITE_HALL_GRADED_JACOBI_OBSTRUCTION_VERIFIED"
PROOF_LABEL = "prop:finite-hall-graded-jacobi"
DEFAULT_FIXTURE = Path("certificates/hall/finite_hall_graded_jacobi")

CLOSURE_FIXTURE = Path("certificates/hall/finite_hall_primitive_commutator_closure")
...
CLOSURE_STATUS = "FINITE_HALL_PRIMITIVE_COMMUTATOR_CLOSURE_OBSTRUCTION_VERIFIED"
...
EXPECTED_COVERAGE = {
    "criterion_count": 1,
    "jacobi_rows": 0,
    "row354_primitive_closure_rows": 0,
...
REQUIRED_OBLIGATIONS = {
    "associative_product_rows",
    "associativity_identity_rows",
    "primitive_closure_rows",
    "primitive_bracket_matrix",
    "parity_rows",
    "jacobi_defect_rows",
    "normal_ordered_bracket_rows",
...
REQUIRED_FIREWALL = {
    ...
    "signed_multiplicity",
    "row354_closure_only",
    "product_without_associativity",
    "bracket_matrix_without_parity",
    "hopf_pairing",
...
TABLE_SPECS = (
    ...
    TableSpec(
        ...
        (
            "criterion_id",
            "associative_graded_algebra_required",
            "homogeneous_parity_required",
            "supercommutator_formula_required",
            "primitive_closure_required",
            "jacobi_formula_recorded",
            "relative_theorem_recorded",
            "current_source_jacobi_proved",
            "primitive_lie_algebra_populated",
            "normal_ordering_claimed",
            "proof_reference",
            "check_status",
            "notes",
        ),
    ...
    TableSpec(
        ...
        (
            ...
            "middle_primitive_basis_id",
            "right_primitive_basis_id",
            "parity_row_id",
            "product_matrix_id",
            "bracket_matrix_id",
            "associativity_identity_row_id",
            "primitive_closure_row_id",
            "jacobi_defect_rank",
            "jacobi_verified",
            "proof_reference",
            "check_status",
...
def check_manifest(fixture: Path) -> None:
    ...
    require_equal(manifest.get("row_number"), 355, "row_number")

    for key in (
        "relative_theorem_recorded",
        "commutator_closure_packet_imported",
        "bialgebra_compatibility_packet_imported",
        "compact_hall_source_ledger_imported",
        "graded_jacobi_formula_recorded",
    ):
        require_true(manifest.get(key), key)

    for key in (
        "product_matrix_rows_supplied",
        "primitive_bracket_matrix_rows_supplied",
        "associativity_rows_supplied",
        "primitive_closure_rows_supplied",
        "jacobi_defect_rows_supplied",
        "parity_rows_supplied",
        "normal_ordered_bracket_rows_supplied",
        "graded_jacobi_proved_for_current_source",
        "primitive_lie_algebra_populated",
    ...
    require_equal(
        ...
        {str(CLOSURE_FIXTURE), str(BIALGEBRA_FIXTURE), str(COMPACT_SOURCE_FIXTURE)},
        "manifest imports",
    )
...
def check_local_tables(fixture: Path) -> None:
    ...
    if PROOF_LABEL not in row.get("proof_reference", ""):
        raise ValueError("criterion proof reference does not point to row-355 proposition")

    require_equal(len(tables["jacobi_rows.csv"]), 0, "Jacobi row count")
...
def check_imports() -> None:
    closure = read_json(CLOSURE_FIXTURE / "manifest.json")
    require_equal(closure.get("status"), CLOSURE_STATUS, "row354 closure packet status")
    require_false(
        closure.get("primitive_commutator_closure_proved_for_current_source"),
        "row354 current-source closure",
    )
    require_false(closure.get("primitive_bracket_populated"), "row354 bracket populated")
    require_table_empty(CLOSURE_FIXTURE / "primitive_closure_rows.csv")
