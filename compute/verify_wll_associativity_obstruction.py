#!/usr/bin/env python3
"""Verify the WLL associativity packet."""
...
SUCCESS_STATUS = "WLL_ASSOCIATIVITY_CONDITIONAL_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "wll_associativity.v1"
...
EXPECTED_ASSOCIATIVITY = {
    "associativity_id": "assoc_WLL_R",
    "word": "WLL",
    "flag_stack_id": "F2_WLL_R",
    "left_comparison_id": "lambda_WLL",
    "right_comparison_id": "rho_WLL",
    "left_composite_id": "m_WL_zeta12_beta_after_m_WL_eta_alpha",
    "right_composite_id": "m_WL_eta_gamma23_after_m_LL_alpha_beta",
    "uses_wrapped_wrapped_product": "false",
...
EXPECTED_FUNCTORIAL = {
    "wll_flag_stack": ("flag_stack", "true"),
    "wll_left_right_maps": ("flag_stack", "true"),
    "wll_local_base_change": ("base_change", "false"),
    "wll_mixed_base_change": ("base_change", "true"),
    "wll_projection_formula": ("projection_formula", "false"),
    "wll_mixed_ts": ("vanishing_cycles", "true"),
}
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check WLL associativity packet.")
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/wll_associativity"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "wll_associativity",
        "hybrid_kind": EXPECTED_KIND,
        "status": SUCCESS_STATUS,
        "word": "WLL",
        "uses_local_local_product": True,
        ...
        "associativity_defect_rank": 0,
        "remaining_word_associativity_count": 4,
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected WLL associativity tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected WLL imports")
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    for index, row in enumerate(table.rows, start=2):
        if row.get("wll_status") != "missing_open_obligation":
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("WLL_ASSOCIATIVITY_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
