#!/usr/bin/env python3
"""Verify the row-284 connected free-E null-trivialization obstruction ledger."""
...
EXPECTED_SCHEMA = "rhomred_free_e_nulltrivialization_obstruction.v1"
EXPECTED_KIND = "rhomred_free_e_nulltrivialization_obstruction"
SUCCESS_STATUS = "RHOMRED_FREE_E_NULLTRIVIALIZATION_OBSTRUCTION_VERIFIED"
PROOF_LABEL = "prop:free-e-borel-nulltrivialization-criterion"
DEFAULT_FIXTURE = Path("certificates/orientation/rhomred_free_e_nulltrivialization")
VANISHING_FIXTURE = Path("certificates/orientation/rhomred_free_e_vanishing")
ORIENTATION_FIXTURE = Path("certificates/orientation/k3e_reduced_orientation")
VANISHING_STATUS = "RHOMRED_FREE_E_VANISHING_OBSTRUCTION_VERIFIED"
...
EXPECTED_COVERAGE = {
    ...
    "criterion_count": 3,
    "free_e_cocycle_count": 0,
    "free_e_zero_count": 0,
    "one_cochain_count": 0,
    "coboundary_defect_zero_count": 0,
    "nulltrivialization_claim": 0,
...
TABLE_SPECS = (
    ...
    TableSpec(
        "nulltrivialization_obstruction_rows.csv",
        (
...
def verify_manifest(fixture: Path) -> None:
    ...
    for key in (
        ...
        "connected_be_h1_zero_recorded",
        "three_retained_strata_inspected",
    ):
        require_equal(manifest.get(key), True, key)
    for key in (
        "free_e_cocycles_supplied",
        "free_e_zero_class_supplied",
        "one_cochains_supplied",
        "coboundary_defect_zero",
        "nulltrivializations_supplied",
...
def verify_imports() -> dict[str, dict[str, str]]:
    ...
    require_equal(vanishing_manifest.get("status"), VANISHING_STATUS, "free-E vanishing packet status")
    for key in (
    ...
    vanishing_rows = read_table_path(
        VANISHING_FIXTURE / "free_e_vanishing_obstruction_rows.csv",
        FREE_E_VANISHING_COLUMNS,
    )
    vanishing_by_id = rows_by(vanishing_rows, "vanishing_id", "imported free-E vanishing rows")
    require_equal(set(vanishing_by_id), {row[0] for row in EXPECTED_ROWS.values()}, "imported vanishing row ids")
    for vanishing_id, row in vanishing_by_id.items():
        for key in ("a1_value", "a2_value", "quotient_borel_row_id"):
            require_equal(row[key], "missing", f"{vanishing_id} imported {key}")
        for key in (
            "free_e_class_computed",
            "a1_zero_verified",
            "a2_zero_verified",
            ...
            require_equal(bool_cell(row, key), False, f"{vanishing_id} imported {key}")
        require_equal(row["check_status"], "verified", f"{vanishing_id} imported check")
...
def verify_sources(tables: dict[str, list[dict[str, str]]]) -> None:
    ...
    require_equal(
        set(rows),
        {
            ...
            "connected_e_descent_lemma",
        },
        "source ids",
    )
    require_equal(rows["free_e_vanishing"]["source_status"], VANISHING_STATUS, "vanishing source status")
    require_equal(
...
def verify_criteria(tables: dict[str, list[dict[str, str]]]) -> None:
    ...
    require_equal(
        set(rows),
        {
            "free_e_cech_borel_nulltrivialization",
            "zero_class_not_enough",
            "h1_be_zero_not_existence",
        },
        "criterion ids",
    )
    for row in rows.values():
        check_verified(row, "criterion_rows.csv")
        require_equal(bool_cell(row, "cocycle_representative_required"), True, f"{row['criterion_id']} cocycle")
        require_equal(bool_cell(row, "zero_class_required"), True, f"{row['criterion_id']} zero")
        require_equal(bool_cell(row, "one_cochain_required"), True, f"{row['criterion_id']} cochain")
        require_equal(
            bool_cell(row, "coboundary_equation_required"),
            True,
            f"{row['criterion_id']} coboundary",
        )
        require_equal(row["h1_be_torsor"], "zero", f"{row['criterion_id']} H1")
        require_equal(
            bool_cell(row, "quotient_orientation_supplied"),
            False,
            f"{row['criterion_id']} orientation",
        )
        require_equal(
            bool_cell(row, "nulltrivialization_supplied"),
            False,
            f"{row['criterion_id']} supplied",
        )


def verify_nulltrivialization_rows(
    tables: dict[str, list[dict[str, str]]],
    vanishing_by_id: dict[str, dict[str, str]],
) -> None:
    rows = rows_by(
        tables["nulltrivialization_obstruction_rows.csv"],
        "nulltrivialization_id",
        "nulltrivialization rows",
    )
    require_equal(set(rows), set(EXPECTED_ROWS), "nulltrivialization row ids")
    for null_id, (vanishing_id, free_e_id, action_id, substack_id, quotient_stack_id) in EXPECTED_ROWS.items():
        row = rows[null_id]
        imported = vanishing_by_id[vanishing_id]
        check_verified(row, "nulltrivialization_obstruction_rows.csv")
        require_equal(row["vanishing_id"], vanishing_id, f"{null_id} vanishing id")
        require_equal(row["free_e_id"], free_e_id, f"{null_id} free-E id")
        require_equal(row["translation_action_id"], action_id, f"{null_id} action")
        require_equal(row["substack_id"], substack_id, f"{null_id} substack")
        require_equal(row["quotient_stack_id"], quotient_stack_id, f"{null_id} quotient")
        require_equal(imported["free_e_id"], free_e_id, f"{vanishing_id} imported free-E")
        require_equal(imported["translation_action_id"], action_id, f"{vanishing_id} imported action")
        require_equal(imported["substack_id"], substack_id, f"{vanishing_id} imported substack")
        require_equal(imported["quotient_stack_id"], quotient_stack_id, f"{vanishing_id} imported quotient")
        for key in (
            "free_e_cocycle_id",
            "free_e_value",
            "one_cochain_id",
...
def verify_inspected_tables(tables: dict[str, list[dict[str, str]]]) -> None:
    ...
    require_equal(
        ...
        {
            "quotient_borel",
            "free_e_vanishing_obstruction_rows",
            "free_e_vanishing_blocked_obligations",
    ...
    require_equal(int_cell(rows["free_e_vanishing_obstruction_rows"], "row_count"), 3, "vanishing row count")
    require_equal(bool_cell(rows["free_e_vanishing_obstruction_rows"], "supplied"), False, "zero supplied")
    require_equal(int_cell(rows["free_e_vanishing_blocked_obligations"], "row_count"), 9, "blocked count")
    require_equal(bool_cell(rows["free_e_vanishing_blocked_obligations"], "supplied"), True, "blocked supplied")
    require_equal(int_cell(rows["orientation_manifest"], "row_count"), 1, "manifest count")
...
def verify_fixture(fixture: Path) -> None:
    ...
    vanishing_by_id = verify_imports()
    tables = {spec.path: read_table(fixture, spec) for spec in TABLE_SPECS}
    verify_sources(tables)
    verify_criteria(tables)
    verify_nulltrivialization_rows(tables, vanishing_by_id)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"RHOMRED_FREE_E_NULLTRIVIALIZATION_OBSTRUCTION_BLOCKED: {exc}", file=sys.stderr)
        return 1
