#!/usr/bin/env python3
"""Verify the finite protected integration coproduct-compatibility packet."""
...
SUCCESS_STATUS = "PROTECTED_INTEGRATION_COPRODUCT_COMPATIBILITY_VERIFIED"
EXPECTED_SCHEMA = "protected_integration_coproduct_compatibility.v1"
EXPECTED_KIND = "protected_integration_coproduct_compatibility"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
PROOF_LABEL = "prop:protected-integration-coproduct-compatibility"
WORDS = ("LL", "LW", "WL", "WW")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check finite protected integration coproduct-compatibility packet."
    )
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hybrid/protected_integration_coproduct_compatibility"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "protected_integration_coproduct_compatibility",
        "hybrid_kind": EXPECTED_KIND,
        "status": SUCCESS_STATUS,
        "protected_integration_defined": True,
        "product_compatibility_imported": True,
        "coproduct_compatibility_certification": True,
        "coproduct_word_count": 4,
        "coproduct_defect_rank": 0,
        "label_coproduct_defect_rank": 0,
    ...
    expected_tables = {
        "coproduct_rows.csv",
        "label_coproduct_rows.csv",
        "integration_square_rows.csv",
        "blocked_obligations.csv",
        "scalar_firewall.csv",
    }
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected coproduct compatibility tables")
    expected_imports = {
    ...
    if set(manifest.get("imports", [])) != expected_imports:
        issues.append("manifest imports do not match expected coproduct compatibility imports")
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("PROTECTED_INTEGRATION_COPRODUCT_COMPATIBILITY_FAILED")
        for issue in issues:
            print(f"- {issue}")
        return 1
