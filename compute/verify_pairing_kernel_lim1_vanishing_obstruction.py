#!/usr/bin/env python3
"""Verify the pairing-kernel lim1 obstruction ledger.

This fail-closed verifier records the data missing from the proof that
R^1 lim vanishes on Hopf-pairing kernel towers.  A positive result
means the obstruction ledger is complete and the core pairing-kernel
...
SUCCESS_STATUS = "PAIRING_KERNEL_LIM1_VANISHING_OBSTRUCTION_VERIFIED"
MANIFEST_NAME = "manifest.json"
README_NAME = "README.md"
EXPECTED_SCHEMA = "pairing_kernel_lim1_vanishing_obstruction.v1"
EXPECTED_KIND = "pairing_kernel_lim1_vanishing_obstruction"
...
REQUIRED_OBLIGATIONS = frozenset(
    {
        ...
        "primitive_space_input",
        "pairing_matrix_rows",
        "pairing_identity_rows",
        "left_kernel_rows",
        "right_kernel_rows",
        "kernel_basis_rows",
        "finite_rank_rows",
        "pairing_transition_input",
        "restricted_kernel_transition_maps",
        ...
        "comparison_kernel_compatibility",
        "no_scalar_pairing_kernel_lim1_substitution",
    }
)
REQUIRED_FIREWALL_ROWS = frozenset(
    {
        ...
        "target_pairing_kernel",
        "signed_multiplicity",
        "denominator_product",
        "scalar_trace",
        "protected_trace",
        "poincare_duality",
        "hopf_pairing_identity_only",
        "empty_ml_status",
    }
)
...
EMPTY_TABLE_SPECS: tuple[EmptyTableSpec, ...] = (
    EmptyTableSpec(
        ...
        (
            ...
            "parity",
            "pairing_side",
            "domain_primitive_space_id",
            "codomain_dual_space_id",
            "matrix_id",
            "proof_reference",
            "check_status",
            "notes",
    ...
    EmptyTableSpec(
        ...
        (
            ...
            "from_window",
            "to_window",
            "source_kernel_space_id",
            "target_kernel_space_id",
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Check pairing-kernel lim1 vanishing obstruction ledger."
    )
    parser.add_argument(
        "--fixture",
        type=Path,
        default=Path("certificates/hall/pairing_kernel_lim1_vanishing"),
    )
...
def check_manifest(fixture: Path, issues: list[str]) -> None:
    ...
    expected = {
        ...
        "fixture_name": "pairing_kernel_lim1_vanishing",
        ...
        "primitive_space_packet_imported": True,
        "transition_radical_packet_imported": True,
    }
    ...
    if set(manifest.get("tables", [])) != expected_tables:
        issues.append("manifest tables do not match expected pairing-kernel tables")
...
def check_empty_tables(fixture: Path, issues: list[str]) -> None:
    for spec in EMPTY_TABLE_SPECS:
        ...
        if not path.is_file():
            issues.append(f"missing pairing-kernel table: {spec.path}")
            continue
        ...
        if rows:
            issues.append(
                ...
                "until pairing-kernel lim1 vanishing is proved"
            )
...
def check_obligations(fixture: Path, issues: list[str]) -> None:
    ...
    if missing:
        issues.append("missing pairing-kernel obligations: " + ", ".join(sorted(missing)))
    if extra:
        issues.append("unexpected pairing-kernel obligations: " + ", ".join(sorted(extra)))
...
def main(argv: list[str]) -> int:
    ...
    if issues:
        print("PAIRING_KERNEL_LIM1_VANISHING_OBSTRUCTION_FAILED", file=sys.stderr)
        for issue in issues:
