...
The seven integral lattices are recorded here in one place: the
hyperbolic plane $U=\mathrm{II}_{1,1}$, the Lorentzian
root lattice $\La_{II}^{2,1}$ of the Borcherds--Kac--Moody algebra
$\autcor$, the K3 cohomology lattice $\La_{K3}=\mathrm{II}_{3,19}$, the
lattice-polarized K3 sublattice \(\Lambda_{\mathrm{pol}}\), the Mukai lattice
$\widetilde H(S,\Z)\simeq\mathrm{II}_{4,20}$, the formal dyonic charge
lattice
\(\Gamma_X^{\mathrm{form}}
=\widetilde H(S,\Z)\oplus\widetilde H(S,\Z)\), and the
signature-$(3,2)$ lattice $\La^{3,2}$ that bridges $\Sp_4(\Z)$ to
the type-IV orthogonal group via the projective exterior-square image.
Each is given by
...
type-II simple roots are checked by the rational computations displayed
below.

The identities about $\Delta_5$, $\autcor$, the orientation character
$\nu_{\Delta_5}$, and the Pfaffian wall normal form on
$W^{(2)}(\La_{II}^{2,1})$ use the lattice data fixed here, not a sign
or scaling chosen later.
...
discriminant $-1$ (Conway--Sloane 1999~\S2.5).  This convention is
primary.

The Borcherds--Gritsenko--Nikulin packaging used in
\S\ref{ssec:exceptional-iso} of Chapter~\ref{ch:singular-theta-lift} and the exterior-square
...
standard convention; a sign flip transports it to the BKM convention without changing rank, discriminant absolute value, or
automorphism group.
\end{convention}
...
\begin{remark}[Standard sign convention versus BKM convention]
\label{rem:U-vs-HypPlan}
The BKM convention uses
$\HypPlan=U(-1)$
...
flips sign.  The lattices $\La^{2,1}$ and $\La^{3,2}$ are presented in
BKM coordinates; even-unimodular ambient lattices
($\La_{K3}$, $\widetilde H(S,\Z)$) are presented in the standard
Conway--Sloane convention.
\end{remark}
...
$2(2\Id_3-\J_3)$ where $\J_3$ is the all-ones matrix.
...
\]
The BKM lattice $\La^{2,1}=\HypPlan\oplus[2]$ in
...
$2(2\Id_3-\J_3)$ is the Gram of the simple roots
...
\]
matching the terminology of Chapter~\ref{ch:pentadic} and the
...
$2(2\Id_3-\J_3)$ of equation~\eqref{eq:bkm-cartan-matrix}; its
determinant is
\[
\det\bigl(2(2\Id_3-\J_3)\bigr)=-32.
\]
...
are integral.  Changing the sign of one hyperbolic basis vector gives
the standard hyperbolic-plane convention \(U=((0,1),(1,0))\) and
presents \(\La^{2,1}\simeq U\oplus\langle 2\rangle\) and
...
\begin{remark}[Identification with \(\La^{2,1}\)]
...
\textnormal{[proved]}
In the coordinates \((f_2,f_3,f_{-2})\) fixed by the Weyl chamber, set
\[
...
$2(2\Id_3-\J_3)$, equal to the displayed Cartan matrix.
...
\cite[(2.7)]{GNII}.  The rational computation gives $\rho=(1,-1/2,1)$ in
$(f_2,f_3,f_{-2})$ coordinates and
$(\rho,\delta_i)=-1$.  Direct computation in the BKM Gram of
Definition~\ref{def:lambda-II-21} gives
\[
(\rho,\rho)
...
\[
\texttt{certificates/lattice/type\_ii\_weyl\_vector}
\]
records the simple-root coordinates, the Cartan matrix
\(4\Id_3-2\J_3\), the discriminant \(-32\), the explicit
\(U(4)\oplus\langle2\rangle\) basis
\(\delta_1+\delta_3,\delta_2+\delta_3,\delta_3\), and the checks
\(\rho=\frac12(\delta_1+\delta_2+\delta_3)\),
\((\rho,\delta_i)=-1\), \((\rho,\rho)=-3/2\), and
\(\rho\in(\La_{II}^{2,1})^\vee\).  The packet is target-side lattice
arithmetic only.

\begin{proposition}[Reflection group; Nikulin's theorem]
\label{prop:nikulin-II21-reflection}
\textnormal{[proved; Nikulin 1980 Theorem~1.4.1]}
...
Gritsenko--Nikulin~\cite[Proposition 1.5]{GN}.
\end{proof}
...
$\delta$-orbit.  Concretely, $s_{\delta_1}(\delta_1)=-\delta_1$,
...
\subsection{The lattice-polarized K3 sublattice \(\Lambda_{\mathrm{pol}}\)}
\label{app:lattice-Lambda-S}

A lattice polarization is a primitive embedding of an integral
lattice \(\Lambda_{\mathrm{pol}}\) into $\La_{K3}$ whose orthogonal complement carries
period structure.

\begin{definition}[Lattice polarization \(\Lambda_{\mathrm{pol}}\hookrightarrow\La_{K3}\)]
\label{def:lattice-polarization}
\textnormal{[definitional]}
A \emph{lattice polarization} of signature $(1,\rho-1)$ on a K3
surface $S$ is a primitive embedding
\[
\Lambda_{\mathrm{pol}}\hookrightarrow\La_{K3}=\mathrm{II}_{3,19},\qquad
\sigma(\Lambda_{\mathrm{pol}})=(1,\rho-1),\quad 1\le\rho\le 19,
\]
where \(\Lambda_{\mathrm{pol}}\subset H^{1,1}(S,\Z)\) contains an ample class.  The
orthogonal complement
\[
\Lambda_{\mathrm{pol}}^\perp\subset\La_{K3},\qquad
\sigma(\Lambda_{\mathrm{pol}}^\perp)=(2,20-\rho),
\]
is the lattice on which the period map of the family
\(\mathcal F_{\Lambda_{\mathrm{pol}}}\to
\mathcal D_{\Lambda_{\mathrm{pol}}^\perp}\) is defined
(Dolgachev~1996; Nikulin 1980~Proposition~1.6.1).
\end{definition}

\begin{verification}[Numerics of \(\Lambda_{\mathrm{pol}}\) and
\(\Lambda_{\mathrm{pol}}^\perp\)]
\label{ver:lambda-S-invariants}
\textnormal{[computed]}
\[
\rk\Lambda_{\mathrm{pol}}=\rho,\quad
\sigma(\Lambda_{\mathrm{pol}})=(1,\rho-1),\quad
\rk\Lambda_{\mathrm{pol}}^\perp=22-\rho,\quad
\sigma(\Lambda_{\mathrm{pol}}^\perp)=(2,20-\rho).
\]
Since \(\La_{K3}\) is unimodular and
\(\Lambda_{\mathrm{pol}}\subset\La_{K3}\) is primitive, Nikulin's
gluing theorem gives an anti-isometry of finite quadratic forms
\[
(A_{\Lambda_{\mathrm{pol}}},q_{\Lambda_{\mathrm{pol}}})
\simeq
(A_{\Lambda_{\mathrm{pol}}^\perp},-q_{\Lambda_{\mathrm{pol}}^\perp}),
\]
hence
\[
|A_{\Lambda_{\mathrm{pol}}}|=
|A_{\Lambda_{\mathrm{pol}}^\perp}|,\qquad
|\det\Lambda_{\mathrm{pol}}|=
|\det\Lambda_{\mathrm{pol}}^\perp|.
\]
The quotient
\(\La_{K3}/(\Lambda_{\mathrm{pol}}\oplus\Lambda_{\mathrm{pol}}^\perp)\)
is the isotropic gluing graph inside
\(A_{\Lambda_{\mathrm{pol}}}\oplus A_{\Lambda_{\mathrm{pol}}^\perp}\);
there is no quotient
\(\Lambda_{\mathrm{pol}}^\perp/\Lambda_{\mathrm{pol}}^*\).
\end{verification}

\begin{proposition}[Nikulin embedding criterion]
\label{prop:nikulin-embedding}
\textnormal{[proved; Nikulin 1980 Theorem~1.14.4]}
A primitive embedding \(\Lambda_{\mathrm{pol}}\hookrightarrow\La_{K3}\) exists if and
...
\[
\rk\Lambda_{\mathrm{pol}}+\ell(A_{\Lambda_{\mathrm{pol}}})
\le 22-\rk\Lambda_{\mathrm{pol}}
\]
...
\(\Lambda_{\mathrm{pol}}^*/\Lambda_{\mathrm{pol}}\) is realized by the orthogonal complement; uniqueness
...
Choose a primitive hyperbolic plane
\(U_{\mathrm{mir}}\subset\Lambda_{\mathrm{pol}}^\perp\).  The
Aspinwall--Morrison--Dolgachev mirror to a
\(\Lambda_{\mathrm{pol}}\)-polarized K3 family is polarized by the
orthogonal complement
\[
\Lambda_{\mathrm{pol}}^\perp
=U_{\mathrm{mir}}\oplus\check\Lambda_{\mathrm{pol}},
\qquad
\check\Lambda_{\mathrm{pol}}
=U_{\mathrm{mir}}^\perp\cap\Lambda_{\mathrm{pol}}^\perp
\simeq\Lambda_{\mathrm{pol}}^\perp\ominus U_{\mathrm{mir}},
\]
the splitting depending on the cusp
\(U_{\mathrm{mir}}\hookrightarrow\Lambda_{\mathrm{pol}}^\perp\)
(Dolgachev 1996~\S5; Aspinwall 1997).  Thus
\(\rk\check\Lambda_{\mathrm{pol}}=20-\rk\Lambda_{\mathrm{pol}}\).
For \(\Lambda_{\mathrm{pol}}=U\oplus\langle -2\rangle\) one obtains
\(\check\Lambda_{\mathrm{pol}}\simeq
U\oplus E_7(-1)\oplus E_8(-1)\), of signature \((1,16)\).  The
hyperbolic plane \(U_{\mathrm{mir}}\) re-enters only in the
Mukai/type-IV comparison, where the period Hodge line is compared
after adjoining the standard Mukai \(U\)-summand.  The assertion that
$\Delta_5^{-2}$ is the corresponding mirror section of the inverse
Hodge line with reduced type-II Heegner support
(View~\textcircled{4}) remains conjectural and is independent of
...
$\mathrm{II}_{4,20}$ (Conway--Sloane 1999~\S16.4).  The
lattice-polarization symbol is \(\Lambda_{\mathrm{pol}}\); the full
Mukai lattice is written \(\widetilde H(S,\Z)\).
\end{verification}

\begin{proposition}[Mukai vector]
\label{prop:mukai-vector}
\textnormal{[proved; Mukai 1987]}
...
$-\rk F\cdot\chi(\mathcal O_S,G)-\rk G\cdot\chi(\mathcal O_S,F)$ term coming from
\(\sqrt{\td(S)}=(1,0,1)\).  Negating the
\(H^0\)--\(H^4\) pairing or dropping the minus sign converts
...
\texorpdfstring{$\Gamma_X^{\mathrm{form}}
=\widetilde H(S,\Z)\oplus\widetilde H(S,\Z)$}{Gamma form}}
\label{app:lattice-formal-dyonic}

The formal additive dyonic lattice on $X=S\times E$ encodes the
electric--magnetic charge data on which the Gram-shadow projection
$\Pi_X$ is defined.  Its two factors are full Mukai lattices, not
lattice-polarization sublattices of \(H^2(S,\Z)\).

\begin{definition}[$\Gamma_X^{\mathrm{form}}$ and its Gram pairing]
\label{def:gamma-X-form}
\textnormal{[definitional; from
...
c=(Q,P)\in\Gamma_X^{\mathrm{form}},\quad Q,P\in\widetilde H(S,\Z).
\]
The Gram pairing
...
\textnormal{[computed]}
\[
\rk\Gamma_X^{\mathrm{form}}=2\cdot\rk\widetilde H(S,\Z)=48,
\qquad
...
=(\operatorname{disc}\widetilde H(S,\Z))^2=1,
\]
...
\[
H^{\mathrm{ev}}(X,\Z)
=H^*(S,\Z)\otimes H^{\mathrm{even}}(E,\Z)
=H^*(S,\Z)\otimes(\Z[1]\oplus\Z[\omega_E]),
\]
where $\omega_E\in H^2(E,\Z)$ is the cohomological point class with
\(\int_E\omega_E=1\).  Every Mukai vector
on $X$ in the formal even sector decomposes as
$v_X=P\otimes 1_E+Q\otimes\omega_E$ with
$Q,P\in\widetilde H(S,\Z)$, and
$\Gamma_X^{\mathrm{form}}=\widetilde H(S,\Z)\oplus\widetilde H(S,\Z)$ is the additive lattice
on which the $(P,Q)$-grading lives
(Definition~\ref{def:formal-even-mukai-sector}).
The tensor K\"unneth pairing on the formal even sector pairs against the
elliptic \(U\)-summand \(H^0(E,\Z)\oplus H^2(E,\Z)\simeq U\), giving
\[
\langle v_X,v_X'\rangle_{\mathrm{Kun}}
=Q\cdot P'+Q'\cdot P,
\]
where each summand is the Mukai pairing on \(\widetilde H(S,\Z)\).  The
direct-sum Gram pairing on \(\Gamma_X^{\mathrm{form}}\) keeps \(Q^2\),
\(Q\cdot P\), and \(P^2\) separately; the resulting genus-two Gram
triple $(Q^2/2,Q\cdot P,P^2/2)$ is the Gram-index data
\(\Pi_X(Q,P)\) used in the Borcherds product
(Theorem~\ref{thm:d6-d2-d0-dictionary-appendixC}).
\end{remark}

\begin{lemma}[Two orthogonal hyperbolic planes inside \(\widetilde H(S,\Z)\)]
\label{lem:two-hyperbolic-planes-appA}
\textnormal{[proved]}
\(\widetilde H(S,\Z)\simeq 4U\oplus 2(-E_8)\) contains four
mutually orthogonal copies of $U$.  In particular, two of them give
\[
U_1\oplus U_2\subset\widetilde H(S,\Z),\qquad U_i\simeq U,\ U_1\perp U_2,
\]
...
\begin{proof}
By Definition~\ref{def:mukai-lattice}, \(\widetilde H(S,\Z)\simeq 4U\oplus 2(-E_8)\)
...
$U\oplus U\simeq\mathrm{II}_{2,2}$ as a sublattice of \(\widetilde H(S,\Z)\).  The
...
\begin{verification}[Formal primitive-lift packet]
\label{ver:formal-primitive-lift-packet-appA}
\textnormal{[computed]}
The packet
\(\texttt{certificates/charge/formal\_primitive\_lift}\) records the
same two-hyperbolic-plane formula.  Its verifier checks
\(\Pi_X(Q,P)=(n,l,m)\), the \(e_1,e_2\) primitive minor, wedge torsion
one, and
\[
\alpha(\Pi_X(Q,P))=2n f_2-l f_3+2m f_{-2}
\]
for the listed formal rows.  The packet is lattice arithmetic in
\(\widetilde H(S,\Z)\); it supplies neither algebraic effectivity nor a
finite HN charge window.
\end{verification}

\subsection{The signature-$(3,2)$ lattice $\La^{3,2}$ and the
$\PSp_4\simeq\SO(3,2)_+$ bridge}
\label{app:lattice-32}

The exceptional isomorphism $\PSp_4(\R)\simeq\SO_+(3,2)$ identifies
the Siegel domain with the type-IV orthogonal domain of an integral
lattice $\La^{3,2}$ of signature $(3,2)$, and it identifies
\(\Sp_4(\Z)/\{\pm\Id\}\) with the exterior-square arithmetic group
\(\Gamma_{3,2}\subset\mathrm P\Orth(\La^{3,2})_+\)
...
unimodular lattice of signature $(3,3)$.  Fix the primitive vector
$q_0:=e_1\wedge e_3+e_2\wedge e_4\in\wedge^2\La^4$, for which
$q_0^2=-2$, and define
\[
...
\textnormal{[computed]}
$\rk\wedge^2\La^4=\binom{4}{2}=6$.  Taking the orthogonal complement of
the primitive vector $q_0$ gives $\rk\La^{3,2}=5$.  The
explicit Gram matrix of $\La^{3,2}$ in the basis
$(f_1,f_2,f_3,f_{-2},f_{-1})$ is
\[
...
in the fixed convention; $\La^{3,2}\simeq U(-1)\oplus
U(-1)\oplus\langle 2\rangle$ in the standard convention]
\label{prop:lambda-32-decomposition}
\textnormal{[proved]}
In the BKM convention of the exterior-square model,
\[
...
\begin{proposition}[The projective exterior-square arithmetic group]
\label{prop:psp4-iso-so32}
\textnormal{[proved; exterior-square model of $\Sp_4(\Z)$]}
The exterior-square representation
$\wedge^2$ induces an isomorphism
\[
...
$\wedge^2(g_0)$, $\wedge^2(g_M)$, $\wedge^2(g_A)$ in the exterior-square model.  The image is the arithmetic group through which
the Siegel modular group acts on the type-IV model.
...
extends to the integral form on the displayed generators.  The
discriminant group of $\La^{3,2}$ has one non-zero element; hence the
stable and full projective components coincide in the
Gritsenko--Nikulin normalization.  This is the content of
\cite[Section~3]{GN} and the exterior-square model of the
automorphic group.
\end{proof}
...
\]
of the Weyl-chamber section is the BKM root lattice: discarding the
...
$\HypPlan=U(-1)$ records the BKM negated convention.
\item $\La_{II}^{2,1}\simeq U(4)\oplus\langle 2\rangle$ (with
$U(4)$ written as \(U(-4)\) in the BKM sign convention):
the BKM root lattice on which the type-II simple-root Cartan
$2(2\Id_3-\J_3)$ and the Weyl vector
...
intersection pairing.  Polarized K3 sublattices \(\Lambda_{\mathrm{pol}}\) embed
primitively into $\La_{K3}$.
\item \(\Lambda_{\mathrm{pol}}\): signature $(1,\rho-1)$ for $\rho\le 19$ (the
algebraic Picard rank).  It is the lattice-polarization sublattice of
\(\La_{K3}\), not the full Mukai lattice.
...
\item
\(\Gamma_X^{\mathrm{form}}
=\widetilde H(S,\Z)\oplus\widetilde H(S,\Z)\): the formal
...
$\PSp_4(\Z)$ to the type-IV orthogonal group via the exterior-square
representation; on this lattice the Borcherds product expansion of
$\Delta_5$ is written.
\end{itemize}

\begin{verification}[End-to-end computation check]
\label{ver:lattice-rational-check}
\textnormal{[computed]}
The lattice data satisfy, in one self-contained $\Q$-arithmetic
computation:
(i) the rank-$5$ Gram matrix $G_{3,2}$ in the
$(f_1,f_2,f_3,f_{-2},f_{-1})$-basis;
(ii) the orthogonality of the symplectic invariant
$q_0=e_1\wedge e_3+e_2\wedge e_4$ to the basis;
...
analysis applies to the ambient $\La^{2,1}$: discriminant $-2$ in the
BKM convention, discriminant $+2$ in the standard convention, and
discriminant group $\Z/2$ generated by $h/2$.  The
type-II sublattice $\La_{II}^{2,1}\subset\La^{2,1}$ of index four has
$|\!\operatorname{disc}\La_{II}^{2,1}|=32$, with discriminant group
$(\La_{II}^{2,1})^*/\La_{II}^{2,1}$ of order $32$
(Verification~\ref{ver:II21-invariants}); the type-II divisibility
statement $(\delta,\La^{2,1})\in\{\Z,2\Z\}$ depends on the order
$2$ of the ambient $\La^{2,1}$ discriminant group, not on the sign
of any determinant.
\end{remark}

\begin{remark}[Cited primary literature]
\label{rem:lattice-primary-lit}
The primary references behind every lattice claim above
