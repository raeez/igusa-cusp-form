...
Vol~II \cite{ChiralBarCobarVolII}: \(\alpha\) primitive,
...
\(\mathcal D_\hbar(\mathrm{CoHA}_{K3\times E})\) of Vol~III
\cite{CYQGVolIII}.  In the universal
...
\cite{ChiralBarCobarVolII} (universal stage chain
...
\paragraph{Cross-volume anchor.}  The output is Vol~II's
\cite{ChiralBarCobarVolII} Construction
...
\(Y^+(X)\to G(X)=D(Y^+(X))\) of Vol~III \cite{CYQGVolIII} requires the Hopf
